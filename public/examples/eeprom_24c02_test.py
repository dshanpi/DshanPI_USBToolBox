...
本脚本演示完整产测流程：扫描 → 读取初始数据 → 写入测试数据 → 回读验证 → 复原。
...
def main() -> int:
    ...
    with I2CTester(client, index=0, speed_khz=speed_khz) as i2c:
        ...
        # 6) 复原原始数据（测试不改坏芯片内容）
        print("[复原] 写回原始数据...")
        i2c.write_reg(EEPROM_ADDR, TEST_ADDR, original,
                      page_size=PAGE_SIZE, write_delay_ms=5)
        ok4, restored, msg4 = read_reg_safe(i2c, EEPROM_ADDR, TEST_ADDR, TEST_LEN, label="[复原] ")
        if ok4 and restored == original:
            print(f"[复原] 完成 [PASS] {restored.hex(' ')}")
        elif ok4:
            print(f"[复原] 警告：复原后与原始不完全一致 {restored.hex(' ')}", file=sys.stderr)
        else:
