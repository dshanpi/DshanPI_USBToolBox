...
// Function to read a 32-bit value from the specified address for ARMv8
static int payloads_readl(const struct sunxi_efex_ctx_t *ctx, const uint32_t addr, uint32_t *val) {
	// payload array containing ARMv8 machine code instructions for reading a value from memory
	...
	// Execute the ARMv832 instructions starting at the given memory address
...
// Function to write a 32-bit value to the specified address for ARMv8
static int payloads_writel(const struct sunxi_efex_ctx_t *ctx, const uint32_t value, const uint32_t addr) {
	// payload array containing ARMv8 machine code instructions for writing a value to memory
	/* Note: Do NOT declare this array as 'static'. Some Windows drivers cannot access payload symbols marked static;
	 * keep it non-static to ensure data visibility when writing to device memory via FEL.
	 */
	return EFEX_ERR_NOT_SUPPORT;
...
struct payloads_ops aarch64_ops = {
		.name = "aarch64",
		.arch = ARCH_AARCH64,
