...
const DEFAULT_MBR_COPY = 1;
const DEFAULT_LOGIC_OFFSET = 0;

export interface CommandModeFlashParams {
  enabled: boolean;
...
export function useCommandModeFlash(params: CommandModeFlashParams) {
  const {
    enabled,
    addLog,
    selectedDevice,
  ...
  const handleStartFlash = useCallback(async () => {
    ...
    await mbrBuilder.setCopy(DEFAULT_MBR_COPY);

    const rawPartition: PartitionInfo = {
      name: 'raw',
      classname: 'DISK',
      address: BigInt(DEFAULT_LOGIC_OFFSET),
    ...
    await mbrBuilder.addPartition(rawPartition);
